% downloaded from 
% https://cvl.psych.ucla.edu/publications/
% April 28 2026
...
%         prioreq = eq/cnormprior;
        ...
        cnormprior = dblquad(inline(priorG1),0,1,0,1,intprecision,@quadl);
...
%             bnormprior = double(int(bpriorf,b,0,1));
            priorG0=sprintf('(exp(-%f*x)*(1-exp(-%f))/%f+exp(-%f*(1-x))*(1-exp(-%f))/%f)',...
             ...
             bnormprior = quadl(inline(priorG0),0,1,intprecision);
...
%             prioreq = eq/cnormprior;
            ...
            priorG1=sprintf('((exp(-%f*(1-y)-%f*x)+exp(-%f*y-%f*(1-x))))',ptemp,ptemp,ptemp,ptemp);
            cnormprior = dblquad(inline(priorG1),0,1,0,1,intprecision,@quadl);
...
%             prioreq = eq/cnormprior; 
            ...
            priorG1=sprintf('(exp(-%f*(1-x)-%f*(1-y))+exp(-%f*(1-x)-%f*(y)))', ptemp,ptemp, ptemp,ptemp);
            cnormprior =  dblquad(inline(priorG1),0,1,0,1,intprecision,@quadl);  
